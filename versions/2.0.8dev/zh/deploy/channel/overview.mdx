...
渠道随[智能体服务](/versions/2.0.8dev/zh/deploy/agent-service)启动，用 `create_app` 的 `channels` 参数声明本服务允许接入哪些渠道类型：
...
不传 `channels` 时，不启用任何渠道类型，渠道功能保持关闭。要接入内置之外的平台，把你的渠道类一并加进列表，见[自定义渠道](/versions/2.0.8dev/zh/deploy/channel/custom)。
...
各接口的完整请求与响应字段，见本章的 [API](/versions/2.0.8dev/en/deploy/openapi.json) 部分。
