...
对于沙箱类后端（Bubblewrap、Docker、E2B、Daytona、K8s、OpenSandbox），MCP 服务器运行在隔离环境*内部*，主机通过工作区内的网关访问它们，详见 [MCP 网关](/versions/2.0.5/zh/building-blocks/workspace/mcp-gateway)。
