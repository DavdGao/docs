...
接入分三步：在[钉钉开放平台](https://open.dingtalk.com/)创建应用并拿到凭据，启动一个智能体服务承载渠道，最后在管理界面添加钉钉渠道。卡片模板已有默认值，开发者只在需要自定义外观时才准备。
...
钉钉的交互式卡片必须先在卡片平台建好模板，渠道运行时只负责填充模板变量。渠道用到的两个模板都预设为钉钉发布的公共模板，**开发者不配置也能直接使用**：审批卡片由渠道在发送时拼出标题、正文与两个按钮，流式回复写入公共 AI 卡片的 Markdown 变量。

想要自定义外观（例如加标签、换按钮配色、调整排版）时，在[卡片平台](https://open-dev.dingtalk.com/fe/card)另建一个模板，把模板 ID 填进渠道配置即可，渠道代码无需改动。
...
流式回复需要模板中有一个 AI 卡片流式组件，把它的模板变量名填到 `streaming_card_key`。使用默认模板时该变量名为 `content`。
