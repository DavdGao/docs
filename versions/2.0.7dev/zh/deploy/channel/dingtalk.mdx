...
工具审批与流式回复开箱可用，两个模板 ID 都已预设为钉钉发布的公共模板：审批卡片由渠道在发送时拼出标题、正文与两个按钮，流式回复写入公共 AI 卡片的 Markdown 变量。**接入钉钉不需要读这一节。**

公共模板的外观是固定的：标题与正文的排版、按钮的配色与宽度都无法调整，也去不掉卡片自带的反馈区。要改这些，在[卡片平台](https://open-dev.dingtalk.com/fe/card)建一个自己的模板，把模板 ID 填进渠道配置，渠道代码无需改动。
