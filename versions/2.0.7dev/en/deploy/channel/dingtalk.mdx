...
Connecting takes three steps: create an app on the [DingTalk Open Platform](https://open.dingtalk.com/) and get its credentials, start an agent service to host the channel, then add the DingTalk channel in the management UI. Both card templates already have defaults, so prepare one only to customise how a card looks.
...
DingTalk interactive cards must be built first in the Card Platform; at runtime the channel only fills in the template variables. Both templates the channel uses default to a template DingTalk publishes, so **a channel works without configuring either**: the approval card gets its title, body, and two buttons laid out at send time, and streaming replies write into the public AI card's Markdown variable.

To customise how a card looks (add a tag, recolour the buttons, rearrange the layout), build a template in the [Card Platform](https://open-dev.dingtalk.com/fe/card) and put its ID in the channel config. No channel code changes.

### Customising the Approval Card
...
### Customising the Streaming Card

Streaming replies need an AI card streaming component in the template. Set its template variable name as `streaming_card_key`; on the default template that name is `content`.
