...
## Customising Card Appearance

Tool approval and streaming replies work out of the box: both template IDs default to a template DingTalk publishes, with the approval card's title, body, and two buttons laid out at send time and streaming replies written into the public AI card's Markdown variable. **Connecting DingTalk does not require reading this section.**

What the public templates look like is fixed. Their layout, button colours, and button widths cannot be adjusted, and the card's built-in feedback row cannot be removed. To change any of that, build a template in the [Card Platform](https://open-dev.dingtalk.com/fe/card) and put its ID in the channel config. No channel code changes.
