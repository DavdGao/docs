...
每个 mode 都遵循**同一套固定的六步评估骨架**，按优先级顺序执行。各 mode 之间的差异仅在于哪些步骤生效，以及最终的兜底行为：

1. **Deny 规则 → DENY** —— 在每个 mode 下都是最高优先级。
2. **Ask 规则 → ASK** —— 命中的 ask 规则会提示用户（在 `DONT_ASK` 下转为 DENY，因为没有用户可以回答）。
3. **只读快速通道 → ALLOW** —— 如果本次调用在事实上是只读的（`check_read_only(tool_input)`），就自动放行。在**所有** mode 下都生效，包括 `DEFAULT`。该判定是逐次调用的，而非静态的工具标记（`git status` 是只读，写文件的 Bash 命令不是）。被标记为命令注入风险的 Bash 命令**不**视为只读，会落到工具的 safety 检查上。
4. **工具 `check_permissions` → ALLOW / DENY / safety ASK / PASSTHROUGH** —— 工具检查本次调用。bypass-immune 的 safety ASK 无法被 allow 规则静默。针对编辑操作的**工作目录自动放行**在这一步发生，且在 `ACCEPT_EDITS` 与 `DONT_ASK` 下生效。
5. **Allow 规则 → ALLOW** —— 命中的 allow 规则放行，除非第 4 步已经触发了 safety ASK。
6. **Mode 兜底** —— 如果以上都没有决断：`DEFAULT` → ASK，`ACCEPT_EDITS` → ASK，`EXPLORE` → DENY，`BYPASS` → ALLOW，`DONT_ASK` → DENY。
...
| Mode | 行为 | 适用场景 |
|------|------|----------|
| `DEFAULT` | 只读调用自动放行（`Read` / `Glob` / `Grep`，以及 `ls`、`git status`、`cat` 等只读 Bash 命令）；其余操作在命中不到 allow 规则时都会提示确认。safety ASK 无法被 allow 规则覆盖 | 最安全，推荐默认值 |
| `ACCEPT_EDITS` | `DEFAULT` 放行的一切，**外加**工作目录内的编辑操作无需提示即自动放行：对已配置工作目录下文件的 `Write` / `Edit`，以及 Bash filesystem 命令（`mkdir`/`touch`/`rm`/`cp`/`mv`/`sed`）**在所有目标路径都在某个工作目录内时** | 用户在场的活跃开发 |
| `EXPLORE` | 放行只读操作；拒绝任何修改。不咨询 allow 规则与工具的 safety 检查 —— 只读保证不能被规则放行掉。用户配置的 DENY / ASK 规则仍然优先于只读自动放行 | 代码探索、规划 |
| `BYPASS` | 完全可信：deny / ask 规则与工具的 DENY 仍然生效，但**工具的 safety ASK 会被跳过**（`rm -rf /`、写入 `~/.bashrc`、命令注入等都会放行），其余一切均放行。请用 deny 规则保护具体路径 | 沙箱环境或完全可信的运行 |
| `DONT_ASK` | `ACCEPT_EDITS` 的无人值守版本：只读自动放行、工作目录内编辑自动放行，但任何原本会提示（不在场的）用户的操作都被转为 **DENY** 而非询问。永不返回 ASK | 无人值守 / 计划任务 |
...
- **危险路径保护** —— `Write`、`Edit`、`Bash` 检查目标文件或命令是否触及敏感路径。返回一个 bypass-immune 的 safety ASK，在 `DEFAULT`/`ACCEPT_EDITS` 下被尊重（allow 规则无法静默），在 `DONT_ASK` 下转为 DENY。`BYPASS` 模式按设计跳过。
- **只读命令检测** —— 只读快速通道解析每一次调用，在**所有 mode**（包括 `DEFAULT`）下自动放行。对于像 `Bash` 这种结果依赖于输入的工具，这一判定通过 `check_read_only()` 方法暴露（详见下文）：`ls` / `git status` / `cat` 是只读，但被标记为命令注入风险的 Bash 命令（如 `ls $(rm -rf /)`）**不**视为只读，会转而落到工具的 safety 检查上。
- **工作目录自动放行** —— 在 `ACCEPT_EDITS` 与 `DONT_ASK` 下，`Write` 与 `Edit` 自动放行已配置工作目录内的文件操作。`Bash` 额外要求 filesystem 命令（`mkdir`/`touch`/`rm`/`cp`/`mv`/`sed` 等）的**所有**目标路径都在某个工作目录内。该自动放行始终从属于上面的 safety 检查 —— 工作目录内的危险操作仍会 ASK / DENY。
...
class MyTool(ToolBase):
    ...
    async def check_read_only(self, tool_input: dict) -> bool:
        ...
        输入时进行重写。引擎在每个 mode 下都会用它执行只读快速通道
...
常见的只读 bash 命令在没有任何规则的情况下也会被自动放行，在**所有 mode**（包括 `DEFAULT`）下都生效。复合命令（`&&`、`||`、`;`、`|`）只有在**所有**子命令都只读时才视为只读。输出重定向（`>`、`>>`）会让命令立即失去只读属性。被标记为命令注入风险的命令（如 `ls $(rm -rf /)`）**不**视为只读，因此不会在此自动放行 —— 它会落到工具的 safety 检查上。
