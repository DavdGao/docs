...
<Tip>
For embedding models and how to configure them, see the [Embedding Model chapter](/versions/2.0.5dev/en/building-blocks/model/embedding); for the service version of RAG (with an HTTP service, file hosting, and distributed indexing), see [RAG Service](/versions/2.0.5dev/en/deploy/rag).
</Tip>
...
See the [Embedding Model chapter](/versions/2.0.5dev/en/building-blocks/model/embedding).
...
<Steps>
  ...
  <Step title="Write to the knowledge base">
    ...
    # Local persistent .db file; requires agentscope[milvuslite]
    ...
    # MongoDB Vector Search (Atlas or self-hosted); requires agentscope[mongodb]
    store = MongoDBStore(
    ...
    # Dense-vector indexes with approximate kNN; requires agentscope[elasticsearch]
...
<CardGroup cols={1}>
  <Card title="RAG Service" icon="server" href="/versions/2.0.5dev/en/deploy/rag" cta="View deployment docs" arrow>
    A multi-tenant, distributed RAG service with HTTP API, file hosting, and managed vector databases.
  </Card>
  <Card title="Middleware" icon="layer-group" href="/versions/2.0.5dev/en/building-blocks/middleware" cta="Learn the middleware mechanism" arrow>
    See how `RAGMiddleware` plugs into the reply / reasoning hooks.
  </Card>
  <Card title="Embedding Model" icon="cube" href="/versions/2.0.5dev/en/building-blocks/model/embedding" cta="View model list" arrow>
