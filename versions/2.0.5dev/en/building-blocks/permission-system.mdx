...
Every mode follows the **same fixed six-step evaluation skeleton**, in priority order. What differs between modes is only which steps are active and the final fallback:

1. **Deny rules → DENY** — highest priority in every mode.
2. **Ask rules → ASK** — a matching ask rule prompts the user (converted to DENY in `DONT_ASK`, where no user is available to answer).
3. **Read-only fast path → ALLOW** — if the specific invocation is factually read-only (`check_read_only(tool_input)`), it is auto-allowed. Active in **every** mode, including `DEFAULT`. The check is per-invocation, not a static tool flag (`git status` is read-only, a Bash command that writes files is not). A Bash command flagged with command-injection risk is **not** treated as read-only, so it falls through to the tool's safety check.
4. **Tool `check_permissions` → ALLOW / DENY / safety ASK / PASSTHROUGH** — the tool inspects the invocation. A bypass-immune safety ASK cannot be silenced by allow rules. **Working-directory auto-allow** for edits happens here and is active in `ACCEPT_EDITS` and `DONT_ASK`.
5. **Allow rules → ALLOW** — a matching allow rule allows, unless a safety ASK already fired in step 4.
6. **Mode fallback** — if nothing above resolved the call: `DEFAULT` → ASK, `ACCEPT_EDITS` → ASK, `EXPLORE` → DENY, `BYPASS` → ALLOW, `DONT_ASK` → DENY.
...
- **Dangerous path protection** — `Write`, `Edit`, and `Bash` check whether the target file or command touches sensitive paths. Returns a bypass-immune safety ASK that is honored in `DEFAULT`/`ACCEPT_EDITS` (allow rules cannot silence it) and converted to DENY in `DONT_ASK`. `BYPASS` mode skips it on purpose.
- **Read-only command detection** — the read-only fast path parses each invocation and auto-allows it in **every** mode (including `DEFAULT`). For input-dependent tools like `Bash`, this is exposed via the `check_read_only()` method (see below): `ls` / `git status` / `cat` are read-only, but a Bash command flagged with command-injection risk (e.g. `ls $(rm -rf /)`) is **not** treated as read-only and instead falls through to the tool's safety check.
- **Working-directory auto-allow** — in `ACCEPT_EDITS` and `DONT_ASK`, `Write` and `Edit` auto-allow operations on files within configured working directories. `Bash` additionally requires that **every** target path of a filesystem command (`mkdir`/`touch`/`rm`/`cp`/`mv`/`sed`, ...) resolves inside a working directory. This auto-allow is always subordinate to the safety checks above — a dangerous op inside a working directory still ASKs/DENYs.
