...
description: 'Using the tuner module to tune agents in multi-agent systems'
icon: 'users'
---
...
This tutorial builds on the concepts introduced in [Overview](/agentscope/1.0.21/en/tune-agent/tune-your-first-agent.mdx) and [Agent Reinforcement Learning](/agentscope/1.0.21/en/tune-agent/model-weights-tuning.mdx). Make sure you are familiar with the core components (Task Dataset, Workflow Function, Judge Function) and the basic tuning workflow before proceeding.
...
<Tip>
For multi-agent scenarios without clear win/loss outcomes, you can still use a separate judge function or LLM-as-a-Judge as described in [Agent Reinforcement Learning — LLM-as-a-Judge](/agentscope/1.0.21/en/tune-agent/model-weights-tuning.mdx#llm-as-a-judge).
</Tip>
...
Add the `logger` parameter to your workflow function (see [Agent Reinforcement Learning — Runtime Monitoring](/agentscope/1.0.21/en/tune-agent/model-weights-tuning#runtime-monitoring)) to debug multi-agent interactions during tuning.
</Accordion>
