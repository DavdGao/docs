...
<Tip>
嵌入模型的介绍和配置方式请见[嵌入模型章节](/agentscope/2.0.4/zh/building-blocks/model)；服务化版本的 RAG（带 HTTP 服务、文件托管、分布式索引）请见 [RAG 服务](/agentscope/2.0.4/zh/deploy/rag)。
</Tip>
...
请见[嵌入模型章节](/agentscope/2.0.4/zh/building-blocks/model)。
...
<CardGroup cols={1}>
  <Card title="RAG 服务" icon="server" href="/agentscope/2.0.4/zh/deploy/rag" cta="查看部署文档" arrow>
    多租户、分布式的 RAG 服务，支持 HTTP API、文件托管、向量数据库托管。
  </Card>
  <Card title="中间件" icon="layer-group" href="/agentscope/2.0.4/zh/building-blocks/middleware" cta="了解中间件机制" arrow>
    了解 `RAGMiddleware` 是如何嵌入 reply / reasoning 钩子的。
  </Card>
  <Card title="嵌入模型" icon="cube" href="/agentscope/2.0.4/zh/building-blocks/model" cta="查看模型列表" arrow>
    可用的嵌入模型及其参数。
  </Card>
</CardGroup>
