...
AgentScope implements different long-term memory capabilities as [agent middleware](/agentscope/2.0.4/en/building-blocks/middleware). Each long-term memory implementation is a `MiddlewareBase` subclass that non-invasively handles memory injection, retrieval, and write-back.
