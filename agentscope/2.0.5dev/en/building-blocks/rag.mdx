...
<Tip>
For embedding models and how to configure them, see the [Embedding Model chapter](/agentscope/2.0.5dev/en/building-blocks/model); for the service version of RAG (with an HTTP service, file hosting, and distributed indexing), see [RAG Service](/agentscope/2.0.5dev/en/deploy/rag).
</Tip>
...
See the [Embedding Model chapter](/agentscope/2.0.5dev/en/building-blocks/model).
...
<Tip>
Milvus Lite dependencies are optional. Install them with `pip install "agentscope[milvuslite]"` when you want to use `MilvusLiteStore`.
</Tip>
...
<Steps>
  ...
  <Step title="Write to the knowledge base">
    ...
    To use Milvus Lite as a local persistent vector store, install `agentscope[milvuslite]` and replace only the vector store construction:
...
<CardGroup cols={1}>
  <Card title="RAG Service" icon="server" href="/agentscope/2.0.5dev/en/deploy/rag" cta="View deployment docs" arrow>
    A multi-tenant, distributed RAG service with HTTP API, file hosting, and managed vector databases.
  </Card>
  <Card title="Middleware" icon="layer-group" href="/agentscope/2.0.5dev/en/building-blocks/middleware" cta="Learn the middleware mechanism" arrow>
    See how `RAGMiddleware` plugs into the reply / reasoning hooks.
  </Card>
  <Card title="Embedding Model" icon="cube" href="/agentscope/2.0.5dev/en/building-blocks/model" cta="View model list" arrow>
