...
flowchart TD
    ...
    E{Check next action} -- exit --> F([Return: waiting<br/>for outside interaction])
...
<Note>
Use `agent.state.reply_id` to reference the reply that is currently paused. See [Human-in-the-Loop](#human-in-the-loop) for how the agent enters a paused state.
</Note>
...
<CardGroup cols={2}>
  <Card title="Permission System" href="/agentscope/2.0.5dev/en/building-blocks/permission-system">
    Control which tools the agent can call and under what conditions.
  </Card>
  <Card title="Middleware" href="/agentscope/2.0.5dev/en/building-blocks/middleware">
