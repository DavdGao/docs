...
See [Plan](/agentscope/2.0.5dev/en/building-blocks/plan) for the full task lifecycle, the storage model, and how to seed or customize tasks programmatically.
...
A backend is selected via the `backend` argument. Backend instances are obtained from a `Workspace`, which defaults to the local environment. See [Workspace](/agentscope/2.0.5dev/en/building-blocks/workspace) for more details.
...
- **`PermissionDecision(..., bypass_immune=True)`** — set on a returned ASK to mark it as a safety check that allow rules cannot silence (e.g. a `DeployTool` flagging `prod-*` targets). See the [safety check contract](/agentscope/2.0.5dev/en/building-blocks/permission-system#safety-check-contract) for per-mode handling.
</Note>
...
This pattern underlies the [human-in-the-loop](/agentscope/2.0.5dev/en/building-blocks/agent) workflow, where certain actions require human approval or manual execution.
...
<CardGroup cols={2}>
  ...
  <Card title="Permission System" icon="shield" href="/agentscope/2.0.5dev/en/building-blocks/permission-system">
    Fine-grained control over which tools can execute and when
  </Card>
  <Card title="Middleware" icon="layer-group" href="/agentscope/2.0.5dev/en/building-blocks/middleware">
    Intercept agent lifecycle hooks — reply, reasoning, model calls, and more
  </Card>
  <Card title="Human-in-the-Loop" icon="user" href="/agentscope/2.0.5dev/en/building-blocks/agent#human-in-the-loop">
