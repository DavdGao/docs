...
The **workspace manager** owns the isolation policy and lifecycle of the [`Workspace`](/agentscope/2.0.5dev/en/building-blocks/workspace) instances the Agent Service hands to each agent. It is the service-side counterpart of the `agentscope.workspace` module: for every workspace subclass there is one matching manager class that provisions, caches, and evicts it.
...
- **Pluggable sandbox backends** — the same lifecycle contract wraps a local directory, a Docker container, an E2B cloud sandbox, an OpenSandbox sandbox, or a Kubernetes Pod. Swapping backend is a one-line change in `create_app`.
...
<Tip>
For an in-depth look at what a workspace *is* (filesystem layout, gateway, MCP wiring, builtin tools), see the [Workspace](/agentscope/2.0.5dev/en/building-blocks/workspace) building-block chapter.
</Tip>
...
The manager is an **async context manager**. The application's `lifespan` enters it on startup and exits it on shutdown through a single `AsyncExitStack`, so any background machinery it owns (e.g. the idle-TTL sweeper in `DockerWorkspaceManager` / `E2BWorkspaceManager` / `OpenSandboxWorkspaceManager` / `K8sWorkspaceManager`) starts and stops in lockstep with the rest of the service:
...
Any workspace class you build against [`WorkspaceBase`](/agentscope/2.0.5dev/en/building-blocks/workspace) can be surfaced to the Agent Service by pairing it with a `WorkspaceManagerBase` subclass. In most cases you only need to fill in a small provisioning + cache scaffold — the isolation policy is inherited.
...
<Tip>
Keep `LocalWorkspaceManager` for local development, `DockerWorkspaceManager` for a single-host production box, and pick E2B, OpenSandbox, or K8s once you scale out.
</Tip>
