...
Agent Team is the multi-agent layer built on top of [Agent Service](/agentscope/2.0.5dev/en/deploy/agent-service). A leader agent — the session the user talks to — can spawn worker agents on demand and exchange messages with them, while every member is just another session with its own state, workspace binding, and event stream. The whole coordination story is expressed through four built-in tools rather than a separate orchestration framework.
...
| `AgentInvite` | Invite an existing agent to join the team. A new session will be created for the invited agent. |
