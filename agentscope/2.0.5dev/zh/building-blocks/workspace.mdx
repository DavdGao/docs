---
title: "工作区"
description: "为 agent 提供工具、skill 与上下文 offload 的执行环境"
---
...
AgentScope 提供四种 workspace 实现 —— 本地文件系统、Docker 容器、E2B 云沙箱、OpenSandbox 沙箱 —— 以及一个 **workspace manager**，在 [Agent Service](/agentscope/2.0.5dev/zh/deploy/agent-service) 中负责分配和追踪 workspace，让多租户部署可以把 workspace 按 user、agent 或 session 维度映射，无需改写 agent 代码。

对 Docker、E2B 与 OpenSandbox 而言，MCP server 跑在隔离环境*内部*；宿主侧通过 workspace 内的 gateway 访问它们，详见后文 [MCP Gateway](#mcp-gateway)。

## 使用 Workspace

### 创建 Workspace

AgentScope 针对四种执行后端提供了对应的 workspace 实现：
...
| `OpenSandboxWorkspace` | 基于 `opensandbox` SDK 的 OpenSandbox 沙箱 |
...
<Tabs>
  ...
  <Tab title="OpenSandboxWorkspace">
    `OpenSandboxWorkspace` 面向 [OpenSandbox](https://github.com/agentscope-ai/opensandbox) 部署 —— 沙箱文件系统本身即持久化层，没有宿主 `workdir`。每个沙箱在 `agentscope.workspace.id` metadata 键下带有 `workspace_id`，重启时 workspace 用该键对 `list_sandbox_infos(...)` 进行过滤，找回并恢复对应的沙箱，磁盘状态原样保留。

    ```
    /workspace/        # 位于沙箱内部（SANDBOX_WORKDIR）
    ├── .mcp           # 注册的 MCP client 配置（JSON 数组）
    ├── data/          # offload 的多模态负载
    ├── skills/        # skill 子目录，每个含 SKILL.md
    └── sessions/      # 每个 session 的 context.jsonl 与工具结果文件
    ...
    workspace = OpenSandboxWorkspace(
        ...
        domain="your-opensandbox-domain",    # 可选，用于自托管部署
        protocol="http",
        timeout_seconds=300,
        extra_pip=["numpy", "pandas"],
        default_mcps=[],
    ...
    首次 `initialize()` 时会在沙箱内部引导（bootstrap）出 gateway 的 venv；`extra_pip` 中的包会连同 gateway 的基础依赖一并装入其中。由于 slim 基础镜像在冷启动时需要串行执行 `apt-get` + `uv` + `pip` 好几分钟，引导命令会以更长的单命令超时运行 —— 除非你确定镜像已预先置备，否则请保持 `request_timeout_seconds` 为默认值（与引导预算相匹配）。
  </Tab>
</Tabs>
...
四种实现共享同一份 `WorkspaceBase` 接口，因此同一份 agent 代码可以在任意后端上无差别运行。接口按使用角色分为四组：
...
Workspace manager 是多租户服务中 workspace 的分配器与生命周期持有者，由 [Agent Service](/agentscope/2.0.5dev/zh/deploy/agent-service) 使用，负责把请求路由到正确的 workspace 实例并在关闭时回收。
...
AgentScope 提供四种 manager，分别配合一种 workspace 后端，默认全部按 agent 隔离：
...
| `OpenSandboxWorkspaceManager` | `OpenSandboxWorkspace` | `agent_id`（沙箱 metadata，无宿主 workdir） | `workspace_id` |
...
要换用其他隔离策略（按 user、按 session 或混合），继承 `WorkspaceManagerBase` 并按自己的键重写 `get_workspace` / `create_workspace` —— 关于 manager 如何接入服务请求生命周期，详见 [Agent Service · Workspace 实现与隔离](/agentscope/2.0.5dev/zh/deploy/agent-service#workspace-实现与隔离)。

<Tip>
Agent Service 在 lifespan 期间把 workspace manager 绑定到 FastAPI 应用状态上，所有请求共享；router 通过 `get_workspace_manager` 依赖注入获取 workspace。完整集成方式见 [Agent Service](/agentscope/2.0.5dev/zh/deploy/agent-service)。
</Tip>

## MCP Gateway

`DockerWorkspace`、`E2BWorkspace` 与 `OpenSandboxWorkspace` 无法直接在宿主侧注册 MCP client —— MCP server 跑在容器或沙箱内部，stdio 会话也无法跨越该边界。AgentScope 用一套 **MCP gateway** 抽象解决此问题：在 workspace *内部*运行一个轻量 FastAPI 进程，它持有上游 MCP 会话，并通过单一带鉴权的 HTTP 端点对宿主暴露这些会话。
...
正因这层抽象，agent 侧代码在每一种 workspace 后端上完全一致 —— 无论上游 MCP 会话在宿主侧（`LocalWorkspace`）还是在隔离环境内（`DockerWorkspace` / `E2BWorkspace` / `OpenSandboxWorkspace`），workspace 的 `list_mcps()` 都返回 `MCPClient` 实例。

<Note>
网关**不会**在宿主可达的网络端口上对外暴露。每一次从宿主到网关的调用其实都发生在沙箱*内部* —— `GatewayMCPClient` 会把请求作为一条 `curl` 命令，通过对应沙箱后端的 `exec_shell` 在沙箱内执行，因此网关始终只监听沙箱自身的 loopback。由于沙箱不对外暴露任何监听服务，这一设计避免了对外开放网关端口所带来的攻击面。
</Note>

## 延伸阅读

<CardGroup cols={2}>
  <Card title="Agent" icon="robot" href="/agentscope/2.0.5dev/zh/building-blocks/agent">
    Agent 抽象、ReAct 循环与 offloader 集成
  </Card>
  <Card title="Tool" icon="wrench" href="/agentscope/2.0.5dev/zh/building-blocks/tool">
    内置 tool、MCP 集成与 toolkit 组装
  </Card>
  <Card title="Agent Service" icon="server" href="/agentscope/2.0.5dev/zh/deploy/agent-service">
    驱动 workspace manager 的多租户服务
  </Card>
  <Card title="Context" icon="database" href="/agentscope/2.0.5dev/zh/building-blocks/context">
    上下文压缩与 offload 触发条件
  </Card>
</CardGroup>
