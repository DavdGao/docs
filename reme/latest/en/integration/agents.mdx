...
Agent integrations sit above a running ReMe service. First [deploy a service](/reme/latest/en/integration/services), then choose how the Agent will connect to it and when it should recall or record memory.
...
Install ReMe through [Quick Start](/reme/latest/en/quickstart), then keep the MCP service running:
...
Conversation refinement and consolidation require the LLM environment described in [Quick Start](/reme/latest/en/quickstart#5-enable-memory-refinement-when-needed).
