---
title: "Plugins"
description: "Connect supported agents to ReMe with in-process Python, tools, Skills, plugins, and maintained integrations."
---

This page covers maintained host integrations. Claude Code, Hermes, OpenClaw, Codex, and other CLI-capable agents connect to a [deployed ReMe service](/reme/latest/en/integration/deployment); QwenPaw uses the in-process Python path. For AgentScope's `ReMeMiddleware`, use the [Python integration](/reme/latest/en/integration/python-sdk#agentscope).
...
Recall uses a short independent timeout before model calls. Completed turns are queued for a serial background writer, and shutdown gets a bounded drain interval. Health, retrieval, and recording cooldowns are isolated so one failing path does not unnecessarily disable the others. See the [Hermes plugin README](https://github.com/agentscope-ai/ReMe/blob/main/plugins/hermes_agent/README.md) for configuration fields and lifecycle details.

## QwenPaw

QwenPaw's recommended path is to embed ReMe through its Python API so the host can reuse the application lifecycle and model configuration while keeping memory local and file-based. Follow the [Python SDK guide](/reme/latest/en/integration/python-sdk) and keep exactly one runtime owner for the workspace.
