...
This page covers service-based agent integrations. First [deploy a ReMe service](/reme/latest/en/integration/deployment), then choose how the agent will connect and when it should recall or record memory. For AgentScope's in-process `ReMeMiddleware`, use the [Python integration](/reme/latest/en/integration/python-sdk#agentscope) instead.
