...
Use the default HTTP service for CLI, scripts, and ordinary applications. Use MCP for agents with native MCP support. Both expose the same jobs and share the workspace. See [Overview](/reme/latest/en/integration/overview) to compare the integration boundaries.
