...
本页介绍持续维护的宿主接入方案。Claude Code、Hermes、OpenClaw、Codex 和其他可执行 CLI 的 Agent 连接[已部署的 ReMe 服务](/reme/latest/zh/integration/deployment)；QwenPaw 使用进程内 Python 路径。AgentScope 的 `ReMeMiddleware` 应使用 [Python 接入](/reme/latest/zh/integration/python-sdk#agentscope)。
