...
本页介绍使用共享服务的 Agent 接入。请先[部署 ReMe 服务](/reme/latest/zh/integration/deployment)，再选择 Agent 如何连接，以及何时召回或记录记忆。AgentScope 的进程内 `ReMeMiddleware` 应使用 [Python 接入](/reme/latest/zh/integration/python-sdk#agentscope)。
